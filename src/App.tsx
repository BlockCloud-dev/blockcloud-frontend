...
function App() {
  ...
  const {
    ...
    setConnections, // 프로젝트 로드 시 연결 정보 설정용
    deleteConnection,
  ...
  const handleLoadProject = (projectData: ProjectData) => {
    console.log('🔄 Project loading started:', {
    ...
    console.log('✅ Project loaded successfully:', {
      name: projectData.name,
      loadedBlocks: projectData.blocks.length,
      loadedConnections: projectData.connections.length
    });
  };

  const handleNewProject = () => {
    setDroppedBlocks([]);
    setConnections([]); // 연결 정보도 초기화
    setSelectedBlockId(null);
    setActiveTab('code');
