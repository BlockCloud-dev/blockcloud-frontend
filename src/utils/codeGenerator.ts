...
export function generateTerraformCode(blocks: DroppedBlock[], connections: Connection[] = []): string {
  // 디버깅: 연결 상태 로깅
  ...
  // 스택킹 연결만 별도로 로깅
  const stackingConnections = connections.filter(conn => conn.properties?.stackConnection);
  console.log('🔧 [CodeGenerator] Stacking connections:', stackingConnections.length);
  stackingConnections.forEach(conn => {
    console.log('  🔗', conn.connectionType, 'from:', conn.fromBlockId, 'to:', conn.toBlockId);
  });
...
`;
  ...
  if (subnets.length > 0) {
    ...
    subnets.forEach(subnet => {
      ...
      console.log('🔍 [Subnet] Looking for VPC connection for subnet:', subnet.id);

      const vpcConnection = connections.find(
        conn => conn.fromBlockId === subnet.id && conn.connectionType === 'subnet-vpc'
      );

      console.log('🔍 [Subnet] VPC connection found:', vpcConnection);

      if (vpcConnection) {
        const connectedVpc = blocks.find(block => block.id === vpcConnection.toBlockId);
        console.log('🔍 [Subnet] Connected VPC block:', connectedVpc);
        if (connectedVpc) {
          vpcRef = `aws_vpc.${connectedVpc.id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase()}.id`;
          console.log('✅ [Subnet] VPC reference set to:', vpcRef);
        }
      } else {
        console.log('❌ [Subnet] No VPC connection found, using fallback:', vpcRef);
      }
  ...
  if (ec2Instances.length > 0) {
    ...
    ec2Instances.forEach(ec2 => {
      ...
      const subnetConnection = connections.find(
        conn => conn.fromBlockId === ec2.id && conn.connectionType === 'ec2-subnet'
      );

      if (subnetConnection) {
        const connectedSubnet = blocks.find(block => block.id === subnetConnection.toBlockId);
        if (connectedSubnet) {
          subnetRef = `aws_subnet.${connectedSubnet.id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase()}.id`;
        }
      }

      // 연결된 보안 그룹들 찾기
      const securityGroupConnections = connections.filter(
        conn => conn.fromBlockId === ec2.id && conn.connectionType === 'ec2-security-group'
      );

      let securityGroupRefs: string[] = [];
      securityGroupConnections.forEach(sgConn => {
        const connectedSG = blocks.find(block => block.id === sgConn.toBlockId);
        if (connectedSG) {
          securityGroupRefs.push(`aws_security_group.${connectedSG.id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase()}.id`);
        }
      });
...
`;
  ...
  if (securityGroups.length > 0) {
    ...
    securityGroups.forEach(sg => {
      ...
      const subnetConnection = connections.find(
        conn => conn.fromBlockId === sg.id && conn.connectionType === 'security-group-subnet'
      );
      ...
      if (subnetConnection) {
        ...
        if (connectedSubnet) {
          ...
          const subnetVpcConnection = connections.find(
            conn => conn.fromBlockId === connectedSubnet.id && conn.connectionType === 'subnet-vpc'
          );
          if (subnetVpcConnection) {
            const connectedVpc = blocks.find(block => block.id === subnetVpcConnection.toBlockId);
            if (connectedVpc) {
              vpcRef = `aws_vpc.${connectedVpc.id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase()}.id`;
            }
          }
        ...
        vpcRef = `aws_vpc.${vpcs[0].id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase()}.id`;
      }
      ...
      if (sg.properties.securityRules && sg.properties.securityRules.length > 0) {
  ...
  if (loadBalancers.length > 0) {
    ...
    loadBalancers.forEach(lb => {
      ...
      const subnetConnections = connections.filter(
        conn => conn.fromBlockId === lb.id && conn.connectionType === 'load-balancer-subnet'
      );

      let subnetRefs: string[] = [];
      subnetConnections.forEach(subnetConn => {
        const connectedSubnet = blocks.find(block => block.id === subnetConn.toBlockId);
        if (connectedSubnet) {
          subnetRefs.push(`aws_subnet.${connectedSubnet.id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase()}.id`);
        }
      });
      ...
      // 연결된 보안 그룹들 찾기
      const securityGroupConnections = connections.filter(
        conn => conn.fromBlockId === lb.id && conn.connectionType === 'load-balancer-security-group'
      );
...
`;
      ...
      const ec2Connections = connections.filter(
        conn => conn.fromBlockId === lb.id && conn.connectionType === 'load-balancer-ec2'
      );
...
`;
        ...
        ec2Connections.forEach((ec2Conn, index) => {
          const connectedEC2 = blocks.find(block => block.id === ec2Conn.toBlockId);
          if (connectedEC2) {
            const ec2Id = connectedEC2.id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase();
...
`;
      ...
      console.log('🔧 [EBS] Looking for EC2 connections for volume:', volume.id);

      const volumeConnection = connections.find(
        conn => (conn.fromBlockId === volume.id && conn.connectionType === 'volume-ec2') ||
      ...
      if (volumeConnection) {
        ...
        const ec2BlockId = volumeConnection.fromBlockId === volume.id
        ...
        if (attachedEC2) {
          const ec2Id = attachedEC2.id.replace(/[^a-zA-Z0-9_]/g, '_').toLowerCase();

          // 연결 속성에 따라 디바이스 이름과 설명 결정
          const isBootVolume = volumeConnection.properties?.volumeType === 'boot' ||
