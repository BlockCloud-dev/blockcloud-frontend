...
const convertExternalToInternal = (externalData: any): ProjectData => {
  ...
  const blocks: DroppedBlock[] = (externalData.blocks || []).map((block: any) => ({
    id: block.id,
    type: block.type,
  ...
  const connections: Connection[] = (externalData.connections || []).map((conn: any) => ({
    id: conn.id,
      ...
      try {
        ...
        if (data.blocks && Array.isArray(data.blocks)) {
          ...
          try {
            ...
            const processedBlocks = validatedData.blocks.map((block: DroppedBlock) => ({
              ...block,
              position: parseVector3(block.position)
            }));
