...
import { canDeleteBlockWithStore } from '../../utils/stackingRules';
...
export const PropertiesPanel: React.FC<PropertiesPanelProps> = ({ }) => {
  ...
  const renderDeleteInfo = () => {
    ...
    // 스태킹 스토어에서 해당 블록의 상태 가져오기
    const stackingState = stackingStates.get(selectedBlock.id);
    const hasChildrenStacked = stackingState?.childBlockIds && stackingState.childBlockIds.length > 0;
    ...
    return (
      <div className="mt-4 p-3 bg-gray-700 rounded">
          ...
          {!hasChildrenStacked ? (
            ...
            <div className="text-red-300 text-sm">
              ...
              <p className="text-gray-300 text-xs">
                위에 스택된 {stackingState.childBlockIds.length}개의 블록을 먼저 이동해야 합니다
              </p>
            </div>
          )}
        </div>
        ...
        {/* 스택된 블록 목록 (이동과 삭제 공통) */}
        {hasChildrenStacked && stackingState?.childBlockIds && (
          <div className="mt-3 pt-2 border-t border-gray-600">
            <p className="text-xs text-gray-400 mb-1">위에 스택된 블록들:</p>
            <div className="text-xs space-y-1">
              {stackingState.childBlockIds.map((childId) => {
                const childBlock = droppedBlocks.find(b => b.id === childId);
                if (!childBlock) return null;
                return (
                  <div key={childId} className="text-orange-300 flex items-center">
                    <div className="w-1 h-1 bg-orange-400 rounded-full mr-2"></div>
                    {childBlock.type} ({childId.substring(0, 8)})
                  </div>
                );
              })}
            </div>
