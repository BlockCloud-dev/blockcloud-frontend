...
interface Canvas3DProps {
    ...
    onBlockPropertiesChange?: (blockId: string, properties: Partial<DroppedBlock['properties']>) => void;
...
export function Canvas3D({
    ...
    const handleBlockMove = (blockId: string, newPosition: Vector3) => {
        ...
        if (isOutsideCanvas) {
            ...
            if (onDeleteConnectionsForBlock) {
                ...
                console.log('🔗 캔버스 이탈로 인한 연결 해제:', blockId.substring(0, 8));
            }
        } else {
            // 스택 해제 감지
    ...
    const checkStackingAndDisconnect = (movedBlockId: string, newPosition: Vector3) => {
        ...
        const unstackedBlocks = previouslyStackedBlocks.filter(prevBlock => {
            return !overlappingBlocks.some(newBlock => newBlock.id === prevBlock.id);
        });

        console.log('🔍 스택킹 상태 변화:', {
            movedBlock: movedBlockId.substring(0, 8),
            previouslyStacked: previouslyStackedBlocks.map(b => ({ id: b.id.substring(0, 8), type: b.type })),
            currentlyOverlapping: overlappingBlocks.map(b => ({ id: b.id.substring(0, 8), type: b.type })),
            unstacked: unstackedBlocks.map(b => ({ id: b.id.substring(0, 8), type: b.type }))
        });
        ...
        if (unstackedBlocks.length > 0 && onDeleteConnection) {
            unstackedBlocks.forEach(unstackedBlock => {
                ...
                const connectionsToDelete = connections.filter(conn =>
                    (conn.fromBlockId === movedBlockId && conn.toBlockId === unstackedBlock.id) ||
                    (conn.fromBlockId === unstackedBlock.id && conn.toBlockId === movedBlockId)
                );
        ...
        if (overlappingBlocks.length > 0) {
            const sortedBlocks = [movedBlock, ...overlappingBlocks].sort((a, b) => a.position.y - b.position.y);
    ...
    const handleWheel = (event: React.WheelEvent) => {
        ...
        if (currentDragData) {
            ...
            const blocksAtPosition = droppedBlocks.filter(block =>
                arePositionsOnSameGrid(new Vector3(block.position.x, 0, block.position.z), new Vector3(snappedX, 0, snappedZ))
            );

            if (blocksAtPosition.length > 0) {
                // 멀티 subnet 프리뷰 처리
                if (currentDragData.id === 'subnet') {
                    const vpcInPosition = blocksAtPosition.find(block => block.type === 'vpc');
                    if (vpcInPosition) {
                        const existingSubnets = blocksAtPosition.filter(block => block.type === 'subnet');
                        ...
                        y = vpcInPosition.position.y + vpcHeight / 2 + subnetHeight / 2 + 0.02;

                        console.log('🎯 PREVIEW Subnet 최적 위치:', {
                            vpcId: vpcInPosition.id.substring(0, 8),
                            existingSubnets: existingSubnets.length,
                            optimizedPos: { x: snappedX.toFixed(3), y: y.toFixed(3), z: snappedZ.toFixed(3) }
                        });
                    } else {
                        // VPC가 없으면 일반 스택킹 처리
                        const sortedBlocks = blocksAtPosition.sort((a, b) => a.position.y - b.position.y);
                        ...
                        const newBlockHeight = getBlockHeight(currentDragData.id, currentDragData.size);
            ...
            console.log('🎯 Calling onDragPreview with position:', position, 'blockData:', currentDragData);
    ...
    const findOptimalSubnetPosition = (
        ...
        // 목표 위치를 VPC 내부로 제한
        let candidateX = Math.max(vpcBounds.minX, Math.min(vpcBounds.maxX, targetX));
        let candidateZ = Math.max(vpcBounds.minZ, Math.min(vpcBounds.maxZ, targetZ));
        ...
        while (attempts < maxAttempts) {
            ...
            for (const existingSubnet of existingSubnets) {
                ...
                if (distance < minDistance) {
                    ...
                    console.log('⚠️ Subnet 충돌 감지:', {
                        candidatePos: { x: candidateX.toFixed(3), z: candidateZ.toFixed(3) },
                        existingPos: { x: existingSubnet.position.x.toFixed(3), z: existingSubnet.position.z.toFixed(3) },
            ...
            if (!hasCollision) {
                console.log('✅ 적절한 Subnet 위치 발견:', { x: candidateX.toFixed(3), z: candidateZ.toFixed(3), attempts });
                break;
            }
            ...
            // VPC 경계 내로 재조정
            candidateX = Math.max(vpcBounds.minX, Math.min(vpcBounds.maxX, candidateX));
            candidateZ = Math.max(vpcBounds.minZ, Math.min(vpcBounds.maxZ, candidateZ));
    ...
    const handleDrop = (event: React.DragEvent) => {
        ...
        try {
            ...
            const blockSize = blockData.size || blockSizes[blockData.id as keyof typeof blockSizes] || [1, 1, 1];
            ...
            const canStack = (upperType: string, lowerType: string) => {
                ...
                console.log('🔍 DROP 스태킹 규칙 검사:', { upperType, lowerType, canStack: stackingRules[upperType]?.includes(lowerType) });
                return stackingRules[upperType]?.includes(lowerType) || false;
            };

            // 스택 유효성 검사 함수 - 멀티 subnet 지원 대폭 개선
            const isValidStack = (newBlockType: string, stackBlocks: typeof droppedBlocks) => {
                if (stackBlocks.length === 0) {
                    // 빈 위치에서는 VPC만 바닥에 배치 가능
                    return { valid: newBlockType === 'vpc', insertIndex: 0, sortedStack: [] };
                }

                // 스택을 Y 위치 기준으로 정렬 (아래부터 위로)
                const sortedStack = [...stackBlocks].sort((a, b) => a.position.y - b.position.y);

                console.log('🔍 DROP 현재 스택 상태:', sortedStack.map(b => ({
                    type: b.type,
                    id: b.id,
                ...
                if (newBlockType === 'subnet') {
                    ...
                    if (vpcInStack) {
                        console.log('🏗️ Subnet 멀티 스택킹 처리 - VPC 발견:', vpcInStack.id);

                        // VPC 위의 모든 subnet 찾기
                        const subnetsOnVpc = sortedStack.filter(block =>
                            block.type === 'subnet' &&
                            Math.abs(block.position.y - (vpcInStack.position.y + getBlockHeight('vpc', vpcInStack.size) / 2 + getBlockHeight('subnet', blockSize) / 2)) < 0.1
                        );

                        console.log('🔢 VPC 위의 기존 subnet 개수:', subnetsOnVpc.length);

                        // VPC 바로 위에 새 subnet 배치 허용 (여러 개 가능)
                        const insertIndex = sortedStack.findIndex(block => block.type === 'vpc') + 1;
            ...
            console.log('🔍 겹치는 블록들:', overlappingBlocks.map(b => ({ id: b.id, type: b.type, y: b.position.y })));
            ...
            if (overlappingBlocks.length > 0) {
                ...
                if (stackValidation.valid) {
                    ...
                    // Y 위치 계산 - 스택 중간 삽입 지원 (기존 로직)
                        ...
                        // 기존 블록들 사이/위에 배치
                        const lowerBlock = stackValidation.sortedStack[stackValidation.insertIndex - 1];
                        const lowerBlockHeight = getBlockHeight(lowerBlock.type, lowerBlock.size);
                        const newBlockHeight = getBlockHeight(blockData.id, blockSize);

                        // 아래 블록 위쪽에 새 블록 배치
                        y = lowerBlock.position.y + lowerBlockHeight / 2 + newBlockHeight / 2 + 0.01;

                        // 위쪽 블록들이 있다면 Y 위치 재계산 필요
                        if (stackValidation.insertIndex < stackValidation.sortedStack.length) {
                            console.log('📏 DROP 위쪽 블록들의 Y 위치 재계산 필요 (중간 삽입)');
                            // Note: 실제 구현에서는 위쪽 블록들의 위치를 업데이트해야 함
                        }
                    }
                ...
                if (blockData.id === 'vpc') {
                    // VPC는 바닥에 배치 가능
                    y = getBlockYPosition(blockData.id, blockSize);
                    console.log('📍 VPC를 바닥에 배치:', { blockType: blockData.id, yPosition: y });
                } else {
        ...
        >
            ...
            >
                ...
                {droppedBlocks.map((block) => {
                    ...
                    const snapSize = 0.5;
                    const blocksAtSamePosition = droppedBlocks.filter(otherBlock =>
                        otherBlock.id !== block.id &&
                        Math.round(otherBlock.position.x / snapSize) * snapSize === Math.round(block.position.x / snapSize) * snapSize &&
                        Math.round(otherBlock.position.z / snapSize) * snapSize === Math.round(block.position.z / snapSize) * snapSize
                    );

                    const isStacked = blocksAtSamePosition.length > 0;

                    // 스택 레벨 계산 (Y 위치 기준으로 정렬하여 순서 결정)
                    const allBlocksAtPosition = [block, ...blocksAtSamePosition]
                        .sort((a, b) => a.position.y - b.position.y);
                    const stackLevel = allBlocksAtPosition.findIndex(b => b.id === block.id);

                    // EBS 역할 분석 (volume 타입만)
                    const ebsRole = block.type === 'volume'
                    ...
                    return (
                        <BaseBlock
                            ...
                            color={blockColors[block.type as keyof typeof blockColors] || '#6b7280'}
                            size={block.size || blockSizes[block.type as keyof typeof blockSizes] || [1, 1, 1]}
                            ...
                            onClick={() => {
                                ...
                                if (isConnecting && connectingFrom && connectingFrom !== block.id) {
                                    // 연결 모드일 때: 연결 완료
                                    console.log('🔗 [CANVAS3D] Completing connection to:', block.id.substring(0, 8));
                                    onConnectionComplete?.(block.id);
                                } else {
                                    // 일반 모드일 때: 블록 선택
                                    console.log('🔗 [CANVAS3D] Normal block selection:', block.id.substring(0, 8));
                ...
                {connections.map((connection) => {
                    const fromBlock = droppedBlocks.find(b => b.id === connection.fromBlockId);
                    const toBlock = droppedBlocks.find(b => b.id === connection.toBlockId);
                ...
                {(() => {
                    const stackConnections: { fromBlock: DroppedBlock; toBlock: DroppedBlock; stackLevel: number }[] = [];
                ...
                {isDraggingBlock && dragPosition && (
                    (() => {
                        ...
                        const blocksAtPosition = droppedBlocks.filter(block =>
                            ...
                            Math.round(block.position.x / snapSize) * snapSize === snappedX &&
                ...
                {isDropPreview && previewPosition && previewBlockData && (
                    (() => {
                        console.log('🎨 Rendering DragPreview:', {
                            ...
                            availableBlocks: droppedBlocks.map(b => ({ id: b.id, type: b.type, position: b.position }))
                        });
                        ...
                        const blockSize = blockSizes[previewBlockData.id as keyof typeof blockSizes] || [1, 1, 1];
                        const blockColor = blockColors[previewBlockData.id as keyof typeof blockColors] || '#6b7280';

                        // 충돌 검사 및 스택킹 위치 계산 (0.5 단위 스냅)
                        const snapSize = 0.5;
                        const snappedX = Math.round(previewPosition.x / snapSize) * snapSize;
                        const snappedZ = Math.round(previewPosition.z / snapSize) * snapSize;
                        ...
                        const canStackPreview = (upperType: string, lowerType: string) => {
                            ...
                            console.log('🔍 PREVIEW 스태킹 규칙 검사:', { upperType, lowerType, canStack: stackingRules[upperType]?.includes(lowerType) });
                            return stackingRules[upperType]?.includes(lowerType) || false;
                        };

                        // 스택 유효성 검사 함수 - DROP과 동일한 로직
                        const isValidStackPreview = (newBlockType: string, stackBlocks: DroppedBlock[]) => {
                            if (stackBlocks.length === 0) {
                                return { valid: newBlockType === 'vpc', insertIndex: 0, sortedStack: [] };
                            }
                        ...
                        console.log('🔍 PREVIEW 겹치는 블록들:', overlappingBlocks.map(b => ({ id: b.id, type: b.type, y: b.position.y })));

                        // 겹치는 블록이 있는 경우 전체 스택 검사
                        if (overlappingBlocks.length > 0) {
                            const stackValidation = isValidStackPreview(previewBlockData.id, overlappingBlocks);
                            ...
                            if (stackValidation.valid) {
                                ...
                                if (stackValidation.insertIndex === 0) {
                                    // 맨 아래 배치 (VPC만 가능)
                                    const blockHeight = getBlockHeight(previewBlockData.id, blockSize);
                                    const newY = blockHeight / 2 + (previewBlockData.id === 'vpc' || previewBlockData.id === 'subnet' ? 0 : 0.1);
                                    finalPreviewPosition = new Vector3(snappedX, newY, snappedZ);

                                    console.log('📏 PREVIEW 맨 아래 배치:', { blockType: previewBlockData.id, y: newY });
                                } else {
                                    // 기존 블록들 사이/위에 배치
                                    const lowerBlock = stackValidation.sortedStack[stackValidation.insertIndex - 1];
                                    const lowerBlockHeight = getBlockHeight(lowerBlock.type, lowerBlock.size);
                                    const newBlockHeight = getBlockHeight(previewBlockData.id, blockSize);

                                    // 아래 블록 위쪽에 새 블록 배치
                                    const newY = lowerBlock.position.y + lowerBlockHeight / 2 + newBlockHeight / 2 + 0.01;
                                    finalPreviewPosition = new Vector3(snappedX, newY, snappedZ);
                                ...
                                console.log('🔄 PREVIEW 다중 블록 스태킹:', {
                                    ...
                                    stack: stackValidation.sortedStack.map(b => ({ type: b.type, y: b.position.y }))
                                });
                            } else {
                                console.log('❌ PREVIEW 스택 배치 불가능:', previewBlockData.id);
                                hasCollision = true;
                                // 충돌이 있으면 빨간색으로 표시 (기본 위치에 배치)
                                const defaultY = getBlockHeight(previewBlockData.id, blockSize) / 2 + 0.1;
                                finalPreviewPosition = new Vector3(snappedX, defaultY, snappedZ);
                            }
                        } else {
                            // 겹치는 블록이 없는 경우
                            if (previewBlockData.id === 'vpc') {
                                ...
                                finalPreviewPosition = new Vector3(snappedX, newY, snappedZ);
                                console.log('📍 PREVIEW VPC를 바닥에 배치:', { blockType: previewBlockData.id, yPosition: newY });
                            } else {
                                // 다른 블록들은 바닥에 직접 배치 불가
                                console.log('❌ PREVIEW 바닥 배치 불가능 (VPC가 아님):', previewBlockData.id);
                                hasCollision = true;
                                // 충돌이 있으면 빨간색으로 표시 (기본 위치에 배치)
                                const defaultY = getBlockHeight(previewBlockData.id, blockSize) / 2 + 0.1;
                                finalPreviewPosition = new Vector3(snappedX, defaultY, snappedZ);
            ...
            {isDragOver && (
                <div className="absolute inset-0 flex items-center justify-center bg-blue-500 bg-opacity-10 
                       border-2 border-dashed border-blue-500 pointer-events-none z-10">
            ...
            <div className="absolute top-4 left-4 bg-white bg-opacity-90 px-3 py-2 rounded-lg shadow-lg z-20">
                ...
                <p className="text-gray-600 text-sm">50x50 무한 격자 (블록 무제한 배치 가능)</p>
            </div>
            ...
            <div className="absolute bottom-4 right-4 bg-white bg-opacity-90 px-3 py-2 rounded-lg shadow-lg z-20 text-sm">
                <div className="text-gray-600">
                    <div className="flex items-center mb-1">
                        <span className="bg-gray-700 text-white px-2 py-0.5 rounded mr-2 text-xs">클릭</span>
                        <span>블록 선택 및 드래그</span>
                    </div>
                    <div className="flex items-center mb-1">
                        <span className="bg-gray-700 text-white px-2 py-0.5 rounded mr-2 text-xs">드래그</span>
                        <span>블록 이동</span>
                    </div>
                    <div className="flex items-center mb-1">
                        <span className="bg-gray-700 text-white px-2 py-0.5 rounded mr-2 text-xs">우클릭</span>
                        <span>속성 패널 열기</span>
                    </div>
                    <div className="flex items-center mb-1">
                        <span className="bg-gray-700 text-white px-2 py-0.5 rounded mr-2 text-xs">Shift+우클릭</span>
                        <span>연결 모드 시작</span>
                    </div>
                    <div className="flex items-center mb-1">
                        <span className="bg-gray-700 text-white px-2 py-0.5 rounded mr-2 text-xs">Delete</span>
                        <span>선택된 블록 삭제</span>
                    </div>
                    <div className="flex items-center">
                        <span className="bg-gray-700 text-white px-2 py-0.5 rounded mr-2 text-xs">휠</span>
